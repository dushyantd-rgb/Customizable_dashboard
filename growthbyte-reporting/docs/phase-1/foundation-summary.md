...
## Repository-root sign-off correction

The Git repository root is `D:\growthbyte\customizable dashboard`, one directory above the project. GitHub Actions discovery therefore requires the active workflow at repository-root `.github/workflows/ci.yml`. The workflow runs Node commands from `growthbyte-reporting/`, API Poetry commands from `growthbyte-reporting/apps/api`, and MCP Poetry commands from `growthbyte-reporting/apps/mcp`. Its pnpm cache key uses `growthbyte-reporting/pnpm-lock.yaml`.

The obsolete project-nested workflow was removed after job and step-command parity was verified. The tracked `apps/web/tsconfig.tsbuildinfo` build artifact was also removed, and `*.tsbuildinfo` is now ignored.
...
8. **The environment template was reduced to the confirmed inventory.** It contains placeholders for reporting Supabase, approved read-only knowledge-Supabase inspection, Google, Meta, and GLM configuration. JWT, application auth secrets, RLS user configuration, MCP auth tokens, AWS configuration, internal job tokens, and extra encryption keys were removed. Browser-exposed variables contain only public service URLs.
...
13. **CI discovery follows the actual Git root.** The active workflow lives at repository-root `.github/workflows/ci.yml`; all working directories and dependency-cache paths explicitly target the `growthbyte-reporting` subdirectory.
...
| CI repository-root correction   | Passed: YAML parsed and nested/root job and step-command parity was verified           |
